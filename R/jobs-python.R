...
#' Train a Python model using Cloud ML
#'
...
#'
#' @param path File to be used as entrypoint for training.
#'
...
#'
#' gcloud_install()
#' job <- cloudml_train_python("/python_path/")
#' }
#'
...
#' @export
cloudml_train_python <- function(path = "./trainer/",
                          master_type = NULL,
                          flags = NULL,
                          region = NULL,
                          config = NULL,
                          ...
                          dry_run = FALSE)
{
  ...
  #print(paste0("storage ",storage))
  #print(paste0("cloudml_file ",cloudml_file))
  #print(paste0("job_yml ",job_yml))
  ...
  #print(paste0("--job-dir= ",file.path(storage, "staging")))
  #print(paste0("--package-path= ",basename(directory)))
  #print(paste0("--config=  cloudml-model", cloudml_file))
  #test if yaml file exists...
  path_config <- paste0(substr(path, 0, nchar(path)-8),"config.yaml")
  ...
  arguments <- (MLArgumentsBuilder(gcloud)
                ...
                ("--package-path=%s",path)
                ("--module-name=%s.task", basename(path))
                ("--runtime-version=%s", cloudml_version)
                ("--region=%s", region)
                #("--config=%s/%s", "cloudml-model", job_yml)
                #force python3
                #TODO : sub path
                ("--config=%s", path_config)
                #("--")
                #("--train-file=%s","gs://test")
                )

  # submit job through command line interface
  # TODO : echo=FALSE
  gcloud_exec(args = arguments(), echo = FALSE, dry_run = FALSE)
  ...
  output <- gcloud_exec(args = arguments(), echo = FALSE, dry_run = FALSE)
